...
public class PathTestHandler extends HandlerSupport{
	...
	public Object testOnlyReturnJson(){
		...
		try {
			WebContextUtils.getResponse().getWriter().append("ssssssssssssss").flush();
			//return null;
		} catch (IOException e) {
			e.printStackTrace();
		}
	...
	public Object testRestfulAndReturnJson1(){
		...
		jsonMap.put("id", RestfulUtils.getIntParam("id"));
		jsonMap.put("info", RestfulUtils.getStringParam("info"));
		jsonMap.put("rid", WebUtils.getIntParam("rid"));
		jsonMap.put("rname", WebUtils.getStringParam("rname"));
		return jsonMap;
	}
